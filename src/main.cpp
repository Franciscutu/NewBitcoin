// Copyright (c) 2009-2010 Newbitcoin Nakamoto
...
#if defined(NDEBUG)
# error "Newbitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc5d822217e963a6324a82be98a2210c059e500a8a1e67831091724acdcd73fc5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Newbitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Newbitcoin Signed Message:\n";
...
{
    // Newbitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Newbitcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Newbitcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Newbitcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Newbitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Newbitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x56d86b19517fd53bd1f8ccdca82cce04330f17ec15282352cd718cc1b5271a42");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "10/03/2024 - 1710072561 for Marc et Hennessy ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047a2739b783b89d62e11d3d8ec15ad155bc19c10aece914a6b7571d2ab326131b2e0e1dd96a14ca34470c8e0c7586ce844237fec72cf8bf17080bcf5932a391ea") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1710072155;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085932401;

        if (fTestNet)
        {
            block.nTime    = 1710072123;
            block.nNonce   = 386344271;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2da319ab227d2f796af9b0b2684d6d45a2557aec0bf04f3c17566dc35174cda4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc3, 0xb5, 0xde }; // Newbitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NewbitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NewbitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NewbitcoinMiner(CWallet *pwallet)
{
    printf("NewbitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("newbitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NewbitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NewbitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NewbitcoinMiner, pwallet));
}
